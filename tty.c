...
tty_hist_up(void)
{
	...
	if (((tty_hist_start - tty_hist_ptr) & TTY_HIST_BUFMASK) == 0)
		return 0;
	while (1) {
		tty_hist_ptr--;
		if (((tty_hist_start - tty_hist_ptr) & TTY_HIST_BUFMASK) == 0)
			break;
		i = (tty_hist_ptr - 1) & TTY_HIST_BUFMASK;
		if (tty_hist_data[i] == 0)
			break;
	}
	return 1;
}
...
tty_hist_down(void)
{
	if (((tty_hist_end - tty_hist_ptr) & TTY_HIST_BUFMASK) == 0)
		return 0;
	while (1) {
		tty_hist_ptr++;
		if (((tty_hist_end - tty_hist_ptr) & TTY_HIST_BUFMASK) == 0)
			break;
		if (tty_hist_data[(tty_hist_ptr - 1) & TTY_HIST_BUFMASK] == 0)
			break;
	}
	return 1;
}
...
size_t
tty_hist_copy(char *line)
{
	size_t count, ptr;
	int c;

#if 0
	if (((tty_hist_start + tty_hist_used - tty_hist_ptr) &
		TTY_HIST_BUFMASK) == 0)
		return 0;
#endif	
	count = 0;
	ptr = tty_hist_ptr;
	while (1) {		
		c = tty_hist_data[ptr & TTY_HIST_BUFMASK];
		if (c == 0)
			break;
		*line++ = c;		
		count++;
...
void
tty_ins(unsigned int offs, unsigned int c)
{
	unsigned int i;
...
void
tty_refresh(unsigned int start, unsigned int end)
{
	unsigned int x, w, todo;
...
tty_onkey(int key)
{
	...
	if (key < 0x100) {
		...
		if (tty_hist_up()) {
			tty_setline(tty_hist_data +
			    (tty_hist_ptr & TTY_HIST_BUFMASK));
		}
	} else if (key == TTY_KEY_DOWN || key == (TTY_KEY_CTRL | 'N')) {
		if (tty_hist_down()) {
			if (((tty_hist_end - tty_hist_ptr) &
				TTY_HIST_BUFMASK) == 0)
				tty_setline("");
			else
				tty_setline(tty_hist_data +
